...
?>
	...
	<div class="main-content main-frm">
		...
		<?php endif; ?>
		<form action="<?php echo forum_htmlencode($forum_page['form_action']) ?>" accept-charset="utf-8" method="post" class="frm-form">
			<div class="hidden">
				<input type="hidden" value="<?php echo forum_htmlencode($forum_page['csrf_token']) ?>" name="csrf_token" />
			</div>
		...
		<form class="frm-form" method="post" accept-charset="utf-8" action="<?php echo forum_htmlencode($forum_page['form_action']) ?>">
			<div class="hidden">
				<input type="hidden" name="csrf_token" value="<?php echo forum_htmlencode($forum_page['csrf_token']) ?>" />
			</div>
				...
				<fieldset class="mf-set mf-extra set<?php echo ++$forum_page['item_count'] ?><?php echo ($forum_page['item_count'] == 1 ? ' mf-head' : ' mf-extra') ?>">
					...
					<div class="mf-box">
						<div class="mf-field mf-field1">
							...
							<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="question[<?php echo forum_htmlencode($question_id) ?>]" value="<?php echo forum_htmlencode($stop_bots_question['question']) ?>" size="24" maxlength="60" /></span>
						</div>
						<div class="mf-field">
							<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_pun_stop_bots['Management answers'] ?></span></label><br />
							<span class="fld-input"><input type="text" id="fld<?php echo $forum_page['fld_count'] ?>" name="answers[<?php echo forum_htmlencode($question_id) ?>]" value="<?php echo forum_htmlencode($stop_bots_question['answers']) ?>" size="24" maxlength="60" /></span>
						</div>
						<div class="mf-field">
							<span class="submit"><input type="submit" name="update[<?php echo $question_id ?>]" value="<?php echo $lang_pun_stop_bots['Management btn update'] ?>" /> <input type="submit" name="remove[<?php echo forum_htmlencode($question_id) ?>]" value="<?php echo $lang_pun_stop_bots['Management btn remove'] ?>" /></span>
						</div>
					</div>
				</fieldset>
				<?php
